...
class IndexComp extends React.Component {
	...
	componentWillReceiveProps(nextProps){
		...
		if (nextProps.updateData.length != 0 && this.state.initData.length != 0) {
			let obj = [...this.state.data]
			let price = localStorage.getItem('language') == 'zh' ? nextProps.priceETH.eth.cny : nextProps.priceETH.eth.usd
			let unit = localStorage.getItem('language') == 'zh' ? '￥' : '$'
			obj.forEach(el => {
				if (el.name == nextProps.updateData[0]) {
					// console.log('===',nextProps.updateData[1])
					el.volume =(nextProps.updateData[1][0][6] * 1).toFixed(2)
					el.last = nextProps.updateData[1][0][2],
					el.price = (nextProps.updateData[1][0][2] * price).toFixed(8) + unit,
					el.change = ((nextProps.updateData[1][0][2] - nextProps.updateData[1][0][1]) / nextProps.updateData[1][0][1] * 100).toFixed(2)
				}
			})
		...
		if (this.state.data.length > 0) {
			content = (
				<div className="index-middleContent">
					<CoinTabs data={this.state.data} sortData={(val) => this.setState({sortBy: val})}/>
				</div>
			);
    	}
		return (
